...
import { Glossary } from '@modusoperandi/licit-glossary';
import type { EditorRuntime, ImageLike, ToolbarMenuConfig, RecentColor } from '../../src/types';
...
class CustomLicitRuntime {
  ...
  uploadImage(blob: any): Promise<ImageLike> {
    let img: ImageLike;
  ...
  uploadVideo(blob: any): Promise<ImageLike> {
    let img: ImageLike;
    // Note: While looking at the uploadImage() function, it is found that a promise is resolved blindly after 3 seconds. Is it a
    // requirement? If not, then I think it causes two issues, 1. Even if an image upload finishes in 700ms, it will take 3s for
    // resolving the promise. 2. If the image upload takes more than 3s, then the promise will be incorrectly resolved before
  ...
  deleteRecentColorById(id: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      try {
        const response: any = fetch(`${apiUrl}${id}`, {
          method: 'DELETE'
        });
 ...
 */
  ...
  sortByStyleName(styles) {
    return styles.sort((a, b) => a.styleName.localeCompare(b.styleName, undefined, { numeric: true }))
      .map((style) => ({ ...style, docType: 'Default' }));
  }
