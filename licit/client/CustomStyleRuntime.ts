...
// import type { Style } from '@modusoperandi/licit-custom-styles';
...
class CustomStyleRuntime {
   ...
   */
  async saveStyle(style: any): Promise<any[]> {
    try {
   ...
   */
  async renameStyle(oldName: string, newName: string): Promise<any[]> {
    try {
   ...
   */
  async removeStyle(styleName: string): Promise<any[]> {
    try {
   ...
   * @private
   */
  async fetchStyles(): Promise<any[]> {
    let styles: any[];
    try {
      styles = JSON.parse(await GET(this.buildRoute('styles')).then());
    } catch (error) {
